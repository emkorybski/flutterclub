...
class Inviter_Plugin_WindowsLiveLogin
{
    ...
    {
        $o = new Inviter_Plugin_WindowsLiveLogin();
    ...
    public static function initMe($settings = null) {
        if(is_null($settings))
            return;
    ...
    public function processConsentToken($token, $context=null)
    {
        ...
        try {
            $wll_consent_token = Engine_Api::_()->loadClass('Inviter_Plugin_WLLConsentToken');
            $consenttoken = $wll_consent_token->initMe($this,
                                                            @$parsedToken['delt'],
                                                            @$parsedToken['reft'],
                                                            @$parsedToken['skey'],
                                                            @$parsedToken['exp'],
    ...
    {
        $url = self::getSecureUrl() . "logout.srf?appid=" . self::getAppId();
        return $url;
    }
   ...
   *
   */
    private static function mhash($h)
    {
        if (function_exists('mhash')) {
          return mhash(MHASH_SHA256, $h);
        }
