...
class Services_Twitter
{
    ...
    protected function decodeBody($body)
    {
        ...
        if ($this->getOption('format') == Services_Twitter::OUTPUT_XML) {
            ...
            // Modified by webligo developments - using Zend_Json when no
            // json_decode()
            if( function_exists('json_decode') ) {
              $result = json_decode($body);
            } else if( class_exists('Zend_Json') ) {
              $result = Zend_Json::decode($body, Zend_Json::TYPE_OBJECT);
            } else {
              // No json support...
              throw new Services_Twitter_Exception(
                  'JSON support is not available. Please install the JSON PHP extension.',
                  self::ERROR_UNKNOWN
              );
            }
